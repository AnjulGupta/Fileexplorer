...
char const* rootPath; //to store rootpath
size_t dirSize=1024;
...
//"\033[r;cH", Move the cursor to row r, column c.
...
void delDir(string dir){ //recursively remove each link of subdir and files from the src dir
	DIR *dp;
	struct dirent *entry;
	struct stat statbuf;
...
void toggleMode(){
	...
	while(1){
		moveCursor(inputLine,0); //move cursor to r row c col
		printf(":");
		getline(cin>>ws,input); //read command from user and store it in input variable
		...
		if(inputVector[0]=="exit"){	// exit command mode
			...
			else if(inputVector[size-1][0]=='/'){	//1st char of destination input is / (absolute path)
				loc=currentDir+inputVector[size-1];
			}
			else if(inputVector[size-1][0]=='.'){	//1st char of destination input is . (relative path)
				loc=currentDir;
			}
			else{
				clrCMD();
			...
			for(int i=1; i<size-1; i++){	//loop for all src files (1 to n-2), n-1 is destination
